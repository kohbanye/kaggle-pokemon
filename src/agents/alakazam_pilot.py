"""Alakazam ex "Powerful Hand" bespoke rule pilot (ported from public Kaggle
notebook ryotasueyoshi/rule-based-not-psychic-alakazam-best-5th, reached 5th).

Used as a REALISTIC bespoke-piloted opponent for local eval (the coupling problem:
generic greedy_plus can't execute combo decks -- see docs/strategy-2026-07-16.md),
and as a candidate 2nd-deck (P/evo combo, damage = 20 x hand size). Not imported by
src/agents/__init__ (imports cg at load); wired into run_eval lazily (name
``alakazam``) and excluded from ruff/ty like the Archaludon port.
"""
...
# Load deck.csv in the dataset (fail-safe: absent when used as a run_eval opponent,
# where AlakazamAgent injects the deck instead of reading a file).
file_path = "deck.csv"
...
class AlakazamAgent:
    """run_eval Agent-protocol adapter over the bare module-level ``agent`` fn."""
