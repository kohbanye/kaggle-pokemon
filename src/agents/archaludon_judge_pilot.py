"""Archaludon ex + Cinderace bespoke rule-based pilot (ported from the public
Kaggle notebook masamikobayashi/a-sample-archaludon-75-wr-vs-my-1300-starmie).

This module is DELIBERATELY not imported by ``src/agents/__init__`` because it
imports the Linux-only ``cg`` engine at load time (``all_card_data()``). It is
wired into ``scripts/run_eval.py`` lazily (name ``archaludon``) and packaged into
the self-contained submission. Its game plan is coupled to the Archaludon netdeck
(``decklists/archaludon_netdeck.csv``); the pilot is what makes that deck work
(generic ``greedy_plus`` bricks it -- see docs/strategy-2026-07-16.md).
"""
...
def apply_overrides(obs, opt, score, reason):
    ...
    # General anti-deck-out (ANY matchup, not just Crustle). Real ladder losses to
    # non-Crustle mill/stall decked us out because the refill safety below was
    # gated on the Crustle branch: Yz (C/basic stall, t22, deck->0) and FJMT
    # (G/evo/1pz grind, t40, deck->0). Refill with Lillie when about to deck out
    # (large hand shuffled back nets deck cards), and stop burning draw supporters
    # when the deck is low -- unless this turn is already lethal (handled elsewhere).
    if opt.type == OptionType.PLAY:
        dc = my_state(obs).deckCount
        cid2 = option_card(obs, opt)
        cid2 = cid2.id if cid2 else None
        if cid2 == LILLIE and dc <= 4 and my_state(obs).handCount >= dc + 6:
            return 16000, "anti-deckout: Lillie to refill deck"
        if dc <= 5 and cid2 in (EXPLORER, LILLIE):
            return -4000, "anti-deckout: don't draw with low deck"
...
def score_play(obs, opt):
    ...
    if cid == JUDGE:
        # Disruption supporter: shuffle BOTH hands into deck, both draw 4. Against
        # hand-size combo (e.g. Alakazam Powerful Hand = 20 x handCount) a big
        # opponent hand is their damage; Judge shrinks it. Archaludon doesn't need
        # a big hand, so the trade is favorable -- but only fire it deliberately.
        if obs.current.supporterPlayed:
            return -1000, "Supporter already used"
        opp = opp_state(obs)
        dc = my_state(obs).deckCount
        # Judge shuffles our hand back before drawing 4, so it is net-safe on the
        # deck, but guard against pathologically low decks anyway.
        if dc <= 4:
            return -800, "save Judge: low deck"
        # Lethal-turn guard: if we can KO-and-win this turn, just attack; don't
        # shuffle away the hand we may need for the kill.
        attacks = planned_archaludon_attacks(obs)
        opp_act = opp_active_pokemon(obs)
        remaining = len(my_state(obs).prize)
        if opp_act and attacks and prize_value(opp_act) >= remaining and any(
                effective_damage(a["damage"], opp_act) >= opp_act.hp for a in attacks):
            return -600, "save Judge: lethal turn"
        # Core use: shrink a large opponent hand.
        if opp.handCount >= 6:
            return 14000, "Judge: disrupt big opponent hand"
        return -400, "save Judge: opp hand small"
...
# ── Harness adapter ─────────────────────────────────────────────────────────
# Lets scripts/run_eval.py drive this bare-function agent (which keeps state in
# module globals). ``reset`` re-seeds and clears per-game state; ``__call__``
# returns the injected deck on the DECK_REQUEST frame and delegates otherwise.

class ArchaludonJudgeAgent:
    """Wrap the module-level ``agent`` fn behind the run_eval Agent protocol.

    Identical to :class:`ArchaludonAgent` but paired with the Judge flex build
    (``decklists/archaludon_judge.csv``: -1 Boss's Orders, +1 Judge) and the
    Judge disruption scoring added to :func:`score_play` above.
    """
    ...
    def __call__(self, obs_dict):
        if obs_dict.get("select") is None:
            # Reset per-game state and return our deck (ignore file-based deck.csv).
            self.reset(0)
