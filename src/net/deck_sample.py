...
def sample_deck_with_logp(
    ...
    The deck is in CB pick order; ``logp[t]`` is the **factored** log-probability of
    the pick: ``log P(category) + log P(card | category)`` over the legal set (see
    :mod:`src.net.deck_factored`). The category factor is what lets energy be sampled
    as a budget instead of being starved by the flat per-card softmax.
    """
    ...
    while len(deck) < DECK_SIZE:
        ...
        card_logits = net.card_logits_with_state(h, cand_matrix[rows])
        idx, logp = factored_pick(
            net.cat_logits(h), card_logits, cat_of[rows], rng, greedy=False,
        )
