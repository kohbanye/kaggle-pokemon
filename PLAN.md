...
| 2026-07-02 | デッキ探索Step2 | **オンラインridgeサロゲート事前選別**(`--surrogate`: batch×4生成→予測上位+25%ランダム枠のみ実対戦・毎世代再学習, PR#20) | Step3ベースライン(同一設定seed0・サロゲート無) | 100gen対照＋再評価1296局/deck＋held-out 5400局＋NashConv14戦略 | 再評価: sur 0.734±.021 vs base 0.710±.021(+0.024, **CI重複=厳格ゲート未達**) | 提出54255202(PENDING) | **採用(コード, opt-in)/保留(既定ON)** | 効率は明確勝ち: sensible 0.895 vs 0.602・アーカイブ平均 0.394 vs 0.352・**較正 cal_r平均0.94**。coverage 11 vs 13(軽い痩せ=doc予告リスク)。held-out 0.720(gauntlet過適合なし)。**NashConv -0.17=母集団支配**(全13戦略勝ち越し・Nash支持100%・greedy\|metalに0.8; 30局/ペアのノイズ+イン・サンプル性に注意)。seed1確認ペア未実施。results/qd_step2_sur100.json, nashconv_qd.json, heldout_qd_step2.json |
| 2026-07-02 | ラダーA/B | **QD産デッキ2本を同時提出**(greedy操縦; 54255202=qd2_sur, 54255206=qd_step1_best) | greedy+metal 532.8(north star) | — | held-out: 0.720 / 0.753・NashConv: **-0.17** / 0.33 | **qd2_sur 574.8→599.2 / qd_step1_best 445.9** | **qd2_sur=新自己ベスト & NashConv圧勝** | **north star 更新**: qd2_sur 574.8(COMPLETE)→同日 599.2 までドリフト(+66 vs metal 532.8)。予想対決は **NashConv 推しの圧勝**(574.8 vs 445.9, 差129): held-out平均勝率よりも**強い母集団への最悪ケース頑健性がラダーを予測**(まだ1ペア=±200ノイズ内の可能性はあるが、held-out最高のqd_step1_bestがnet_run7+metal(432)並みに沈んだのは示唆的)。§1.3相関: 以後の提出ゲートは NashConv を主・held-out を副に。Step1+3+2 のフルパイプライン産デッキが初めてラダーで実証された。net_run7+metal は 432.2 で確定 |
| 2026-07-02 | デッキ探索Step4 | **共進化gauntlet＋hall-of-fame＋warm-start**(`--rounds`: 毎ラウンド相手=アーカイブtop-4 elites＋HoFサンプル・アーカイブは持ち越し再スコア; src/qd/coevo.py) | Step3ベースライン(qd_step3_heur100, 同一設定・固定gauntlet・同100gen予算) | 100gen対照＋NashConv15戦略(30局/ペア)＋held-out 2700局 | NashConv: **qd4_coevo 0.067=15戦略最小**(最悪マッチ0.467・Nash支持0.91; qd3_rand/qd2_sur 0.133)。held-out: **0.648[.630,.666]=QD勢最弱**(qd_step1_best .753/qd3_rand .722/metal .684未満) | 提出済(PENDING) | **KEEP(コード)/ラダーA/B待ち(デッキ)** | 指標がまた乖離(Step2と同型)→§1.3ゲート(NashConv主)に従い提出=較正3点目。プロセス: admits 69 vs 47(warm-startでも停滞なし・ラウンド頭re-scoreでバー上昇を確認 0.676→0.671→0.667→0.621→0.625)・coverage 12 vs 13・HoFにr1_best等が相手として循環。デッキ=2色/エネ27/prize12。**エンジン稀少バグ発見**: cg が 'invalid index 0>=0' C++ abort でワーカー死(≈15万局に1回)→Pool.map永久ハング→apply_async+timeout化(80c9c52)。raw fitnessはアーム間比較不能(進化gauntletの方が硬い)。results/qd_step4_coevo100.json, nashconv_qd4.json, heldout_qd_step4.json |
| 2026-07-02 | デッキ探索Step4本番 | **プロダクションcoevo**(55r×40g=2200世代・surrogate ON・hof32・eval-timeout45; ~5h) | qd4_coevo(A/B版)・既存QD勢 | NashConv16戦略＋held-out 2700局 | **held-out 0.800[.785,.815]=歴代1位**(qd_step1_best 0.753とCI分離)＋NashConv 0.267=3位/16(Nash支持0.125)=**初の両指標強**。metal系を壊滅させる(greedy|metal最悪相手=本デッキ0.1) | 提出54266227(PENDING) / **qd4_coevo確定431.9=沈没** | **本命候補** | デッキ=単色・エネ15・トレーナー39・ポケモン6のリーン型(初の「通常Standard構成」到達=Step3の宿題解消)。coverage14・admits2046(55ラウンド停滞なし)・abort10/1150万局。**較正3点目で逆転**: NashConv最良/held-out最弱のqd4_coevoがラダー431.9に沈み、held-out最良のqd_step1_bestは480.9に上昇→held-outも情報あり・NashConvは母集団/ノイズでrun間0.067→0.333ブレ→**当面は「両指標で強い」を提出ゲートに**。results/qd_step4_production.json, nashconv_qd4prod.json, heldout_qd4prod.json |
| 2026-07-03 | Step5 スケーリング | **QD↔RL外側ループ＋descriptor v3＋進化ライン変異**(5 outer×[8r×25g QD+300it RL]・~5.5h・coverage 92) | qd4_prod(Step4本番) | NashConv18戦略＋held-out 5400局 | **qd5net\|qd5_rl: NashConv 0.0=18戦略無敗**(最悪0.5・Nash支持0.6)＋**held-out 0.812[.797,.826]=歴代最高**。同デッキgreedy操縦 0.726(**操縦で+8.6pp**) | 2本提出(54297327/54297331, PENDING) / qd4_prod 562.6=north star超え維持 | **本命(共学習ペア)** | v3の容量が機能: coverage 12-14→92、上位セルは進化×toolbox型が独占(メタアンカー全滅)。**per_pilotのnet>greedy格差が「上手い審判が要る」仮説を実証**→RL追随審判＋共学習ペア提出。ユーザ発案のdeck-conditioned net(deck_ctx, ゼロパッド移行・パリティ固定)もマージ済み=次ランでA/B。qd4_coevo 441.3確定。results/qd_step4_production.json(qdrl_run1はdata/), nashconv_qd5.json, heldout_qd5.json |
| 2026-07-05/06 | Step5b/5c 全部入りラン×2 | run2=交叉+racing+ctx(アンカー無), run3=同+**永続アンカー** | qd5/qd4_prod | NashConv 21-23戦略＋held-out | run2: qd6_rl **held-out 0.147/NashConv 1.0=自己参照ドリフトで崩壊**(全8R gauntletにメタデッキ0枚、pilotも汚染0.812→0.761)。run3(接地後): qd7_rl 0.704/0.6=健全だが前線更新なし。**ctx-net(run3)は既知デッキで歴代最高タイ**(qd5_rl 0.816, qd4_prod 0.804) | 提出なし(既存超えず) | **アンカー=必須(採用)** / 交叉+racing=ドリフト交絡で判定持越し / ctx=無害〜同等 | 教訓: 共進化は永続外部アンカー無しで11hで自壊する(build_gauntlet anchor枠で恒久修正 9346c25)。デッキ前線は3ラン連続でqd4_prod(0.800)を超えず=**greedy操縦デッキの天井付近**の可能性。王者不変: local=qd5net|qd5_rl(Nash支持0.75)、ladder=greedy+qd4_prod 562.6。results/nashconv_qd{6,7}.json, heldout_qd{6,7}.json, heldout_qd{6,7}net_pilot.json |
| 2026-07-07 | RL報酬shaping A/B | **potential-based shaping**(prize差0.05+盤面供給差0.03; run5=run4+shapingのみ・run4からwarm-start) | run4 net(baseline) | heldout2フル 2700局×2デッキ×2net＋champion選抜 | **qd5_rl操縦: shaped 0.693[.676,.711] vs base 0.668[.650,.686] = +2.5pp(CI辛うじて分離)** / qd7_r5操縦: 0.715 vs 0.735 = -2pp(重畳) | 提出なし | **KEEP(軽度有効)** | スキル依存デッキでは効く(run4で退行していた操縦がrun1水準に回復)が、greedy親和デッキでは微退行=万能ではない。デッキ側は3ラン連続0.73-0.74プラトー(run5 screen top 0.742=フル換算で壁内)。飽和したRLに対しshapingは「維持・回復」には効くが天井を破る力はまだない。次候補: shaping係数チューニング/値ヘッド改良/長時間RL専用フェーズ。results/heldout2_run{4,5}net.json, champion_run5_h2.json |
| 2026-07-08 | RL容量+スケジュール | **512幅(net2net無傷拡張)+lr 5e-4→5e-5減衰+2000iter専用フェーズ**(rl_long1; アーカイブ固定=run5r7) | run5 net(128幅) | heldout2フル 2700局×3デッキ | qd5_rl **0.665 vs 0.693(-2.8pp)** / qd7_r5 0.719 vs 0.715(タイ) / qd9_r1 0.719 vs 0.710(タイ) | 提出なし | **不採用(飽和原因は容量・lr・報酬・学習時間のいずれでもない)** | 密報酬(shaping)・4倍容量・lr減衰・6.7倍学習量のどれも操縦を改善せず=**自己対戦データ分布の天井**が確定的。論文との差は分布規模(3.2e8サンプル/期)でCPU14コアでは埋まらない。RL投資はここで一区切り: greedy+qd7_r5(667)が提出本線、RLはスキル依存デッキ限定の+2-3pp資産として保持。今後のElo源泉はデッキ/メタ適応(エピソード収集→プール更新→QD)に回帰。results/heldout2_rllong1.json |
